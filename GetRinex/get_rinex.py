...
# Define station file, data source, and date range in which we want to download rinex files

# Station File (Lat,Lon,StaId)
sfile = ("./stations/gps_locations_MJS_PS_NASA.txt" )

# Data source (1=NASA-CDDIS,2=GAGE)
dsource = 1

# Specify Date Range (YYYY,MM,DD,hh,mm,ss)
...
# Datetime object so we can index dates of interest
...
for i in range(0,len(stations)):
    ...
    latx = lat[i]
    lonx = lon[i]
    print('Currently downloading data for station : ', station)
    # Download rinex files
    if (dsource == 1):
        for j in range(0,len(dto)):
            if not (os.path.isdir("/projects/ms180369/gnss-data/" + station + "/data/y" + str(dto[j].year) + "/d" + (format(dto[j],'%j')) + "/")):
                os.makedirs("/projects/ms180369/gnss-data/" + station + "/data/y" + str(dto[j].year) + "/d" + (format(dto[j],'%j')) + "/")
            cwd = os.getcwd()
            ddir = ("/projects/ms180369/gnss-data/" + station + "/data/y" + str(dto[j].year) + "/d" + (format(dto[j],'%j')) + "/")
            os.chdir(ddir)
